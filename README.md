...
- Für CI/CD sollte entsprechend `docker build --build-arg PLUGIN_BUILD_HASH=$(git log -1 --format=%h 2>/dev/null || cat revision.txt 2>/dev/null || echo unknown) ...` verwendet werden.
